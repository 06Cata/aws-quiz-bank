...
```text
完整讀取 question_sources/prompt_saa.md，依照全部規則整理 AWS SAA-C03 的 Q11-Q100。
Q11-Q100 是本次要完成的總範圍；每個 JSON 最多 15 題，請自動連續拆檔，不可把 90 題寫進同一個檔案。
開始前先掃描 questions/saa_Q*-Q*.json 內的 question_no，確認指定的第一題等於現有最後一題加 1，且既有題號沒有重複或缺漏；不符合時停止並回報。
同時查閱題號對應的 with_aizh 與 with_discussion PDF，實際建立所有 JSON 檔案，最後執行 npm run validate:questions，修正到驗證通過。
...
```text
questions/saa_Q11-Q25.json
questions/saa_Q26-Q40.json
questions/saa_Q41-Q55.json
questions/saa_Q56-Q70.json
questions/saa_Q71-Q85.json
questions/saa_Q86-Q100.json
```

指定範圍規則：

- 格式固定為 `saa_Q起始題號-Q結束題號.json`，例如 `saa_Q101-Q160.json`。
- 起始題號必須接在現有正式題庫最後一題之後，不能覆蓋或跳過既有題目。
- 結束題號可以不是 15 的倍數；最後一檔只放剩餘題目。
- SAA-C03 可指定的最高題號是 Q1019。
...
- 正式 SAA 題庫只輸出到 `questions/saa_Q起始-Q結束.json`。
...
開始整理前，先掃描 `questions/saa_Q*-Q*.json`，不能只依檔名猜測最後題號。
...
| PDF | 第一題 | 最後一題 | 題數 |
| :--- | :---: | :---: | ---: |
| `AWS_SAA-C03_with_aizh_part_01.pdf` | Q1 | Q107 | 107 |
| `AWS_SAA-C03_with_aizh_part_02.pdf` | Q104 | Q210 | 107 |
| `AWS_SAA-C03_with_aizh_part_03.pdf` | Q207 | Q312 | 106 |
| `AWS_SAA-C03_with_aizh_part_04.pdf` | Q309 | Q414 | 106 |
| `AWS_SAA-C03_with_aizh_part_05.pdf` | Q411 | Q515 | 105 |
| `AWS_SAA-C03_with_aizh_part_06.pdf` | Q512 | Q619 | 108 |
| `AWS_SAA-C03_with_aizh_part_07.pdf` | Q616 | Q723 | 108 |
| `AWS_SAA-C03_with_aizh_part_08.pdf` | Q720 | Q824 | 105 |
| `AWS_SAA-C03_with_aizh_part_09.pdf` | Q821 | Q926 | 106 |
| `AWS_SAA-C03_with_aizh_part_10.pdf` | Q923 | Q1019 | 97 |

### 2.2 with_discussion 對照表

| PDF | 第一題 | 最後一題 | 題數 |
| :--- | :---: | :---: | ---: |
| `AWS_SAA-C03_with_discussion_1.pdf` | Q1 | Q82 | 82 |
| `AWS_SAA-C03_with_discussion_2.pdf` | Q79 | Q161 | 83 |
| `AWS_SAA-C03_with_discussion_3.pdf` | Q158 | Q241 | 84 |
| `AWS_SAA-C03_with_discussion_4.pdf` | Q238 | Q334 | 97 |
| `AWS_SAA-C03_with_discussion_5.pdf` | Q331 | Q434 | 104 |
| `AWS_SAA-C03_with_discussion_6.pdf` | Q431 | Q538 | 108 |
| `AWS_SAA-C03_with_discussion_7.pdf` | Q535 | Q647 | 113 |
| `AWS_SAA-C03_with_discussion_8.pdf` | Q644 | Q768 | 125 |
| `AWS_SAA-C03_with_discussion_9.pdf` | Q765 | Q892 | 128 |
| `AWS_SAA-C03_with_discussion_10.pdf` | Q889 | Q1019 | 131 |
...
1. `zh` 只能使用簡體中文解析。除 AWS 官方名稱、API 和技術識別字外，不可混入英文選項原句。
...
PDF 中文可能是簡體字、錯誤部件字或 OCR 缺字。輸出前必須對照英文修正為自然、完整的簡體中文。
...
遇到不在對照表內的亂碼時，使用同題英文原文與上下文重建正確簡體中文，不能保留明顯亂碼。
...
```text
questions/saa_Q111-Q125.json
questions/saa_Q126-Q140.json
```
...
若不需要指定較長範圍，只想從現有題庫後面自動接續一批 15 題，可直接使用：

```text
完整讀取 question_sources/prompt_saa.md，依照所有步驟執行。
先掃描 questions 內全部 saa_Q*-Q*.json，找出最後一個 question_no，接著從下一題開始整理 15 題。
同時參考對應的 with_aizh 與 with_discussion PDF，輸出到 questions/saa_Q起始-Q結束.json。
修正簡體中文、OCR 亂碼與雙語解析，移除所有廣告，最後執行 npm run validate:questions，直到驗證通過。
```
